...
static void send_system_configuration(void)
{
	char sys_tick_buff[16];
	int temp_val;
	int err;
	...
	temp_val = snprintf(sys_tick_buff, sizeof(sys_tick_buff),
						",u32,%d", sys_clock_hw_cycles_per_sec());
	if ((temp_val < 0) || ((size_t)temp_val >= sizeof(sys_tick_buff))) {
		return;
	}
